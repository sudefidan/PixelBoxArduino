...
#define BUTTON_PIN 0
#define FILTER_TOGGLE_PIN 21 // Button to cycle through filter modes (adjust pin as needed)
...
void setup()
{
  ...
  // Initialize BLE with device name "ESP32Camera"
  if (initializeBLE("PixelBox"))
  {
...
void loop()
{
  ...
  if (digitalRead(FILTER_TOGGLE_PIN) == LOW || remoteFilterChangeRequest)
  {
    ...
    char message[50];
    sprintf(message, "filter_changed:%d", currentFilterMode);
    notifyBLEClients(message);
  }
